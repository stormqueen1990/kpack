...
const (
	...
	networkWaitLauncherDir = "kpack-network-wait-launcher-dir"
		...
		Spec: corev1.PodSpec{
			...
			Containers: steps(func(step func(corev1.Container, ...stepModifier)) {
				...
				if notaryConfig == nil {
					...
					ifWindows(config.OS, addKpackVolume(), useNetworkWaitLauncher(dnsProbeHost))...)
				}
			}),
			...
			InitContainers: steps(func(step func(corev1.Container, ...stepModifier)) {
				step(
					...
					ifWindows(config.OS, removeDirectExecute(), addKpackVolume(), removeSecurityContext())...,
				)
				step(
					corev1.Container{
						Name:    "detect",
...
func useNetworkWaitLauncher(dnsProbeHost string) stepModifier {
	return func(container corev1.Container) corev1.Container {
		...
		container.Command = []string{"/kpack/network-wait-launcher"}
		return container
	}
}
...
func addKpackVolume() stepModifier {
	return func(container corev1.Container) corev1.Container {
