	...
	when("BuildPod", func() {
		...
		when("builder is windows", func() {
			...
			it("uses windows node selector", func() {
				...
				assert.Equal(t, pod.Spec.NodeSelector, map[string]string{"kubernetes.io/os": "windows"})
			})
			...
			it("configures prepare for windows build init", func() {
				...
				assert.Equal(t, prepareContainer.Name, "prepare")
				assert.Equal(t, prepareContainer.Image, config.BuildInitWindowsImage)
				assert.Equal(t, []string{
					"-basic-git=git-secret-1=https://github.com",
					"-ssh-git=git-secret-2=https://bitbucket.com",
					"-basic-docker=docker-secret-1=acr.io",
				...
				assert.Subset(t, pod.Spec.Volumes, []corev1.Volume{
					{
						Name: "kpack-network-wait-launcher-dir",
						VolumeSource: corev1.VolumeSource{
							EmptyDir: &corev1.EmptyDirVolumeSource{},
						},
					},
			...
			it("configures detect step for windows", func() {
				...
				assert.Equal(t, detectContainer.Name, "detect")
				assert.Subset(t, pod.Spec.Volumes, []corev1.Volume{
					{
						Name: "kpack-network-wait-launcher-dir",
						VolumeSource: corev1.VolumeSource{
				...
				assert.Equal(t, detectContainer.Command, []string{"/kpack/network-wait-launcher"})
				assert.Equal(t, detectContainer.Args, []string{
					dnsProbeHost,
					"--",
			...
			it("configures analyze step", func() {
				...
				assert.Equal(t, analyzeContainer.Command, []string{"/kpack/network-wait-launcher"})
				assert.Equal(t, analyzeContainer.Args, []string{
					dnsProbeHost,
					"--",
			...
			it("configures restore step", func() {
				...
				assert.Equal(t, restoreContainer.Name, "restore")
				assert.Subset(t, pod.Spec.Volumes, []corev1.Volume{
					{
						Name: "kpack-network-wait-launcher-dir",
						VolumeSource: corev1.VolumeSource{
				...
				assert.Equal(t, restoreContainer.Command, []string{"/kpack/network-wait-launcher"})
				assert.Equal(t, []string{
					dnsProbeHost,
					"--",
			...
			it("configures build step", func() {
				...
				assert.Equal(t, buildContainer.Name, "build")
				assert.Subset(t, pod.Spec.Volumes, []corev1.Volume{
					{
						Name: "kpack-network-wait-launcher-dir",
						VolumeSource: corev1.VolumeSource{
				...
				assert.Equal(t, buildContainer.Command, []string{"/kpack/network-wait-launcher"})
				assert.Equal(t, []string{
					dnsProbeHost,
					"--",
			...
			it("configures export step", func() {
				...
				assert.Equal(t, exportContainer.Name, "export")
				assert.Subset(t, pod.Spec.Volumes, []corev1.Volume{
					{
						Name: "kpack-network-wait-launcher-dir",
						VolumeSource: corev1.VolumeSource{
				...
				assert.Equal(t, exportContainer.Command, []string{"/kpack/network-wait-launcher"})
				assert.Equal(t, []string{
					dnsProbeHost,
					"--",
			...
			it("configures the completion container for notary on windows", func() {
				...
				completionContainer := pod.Spec.Containers[0]
				assert.Equal(t, completionContainer.Name, "completion")
				assert.Equal(t, completionContainer.Image, config.CompletionWindowsImage)
				...
				assert.Equal(t, completionContainer.Command[0], "/kpack/network-wait-launcher")
				assert.Subset(t, pod.Spec.Volumes, []corev1.Volume{
					{
						Name: "kpack-network-wait-launcher-dir",
						VolumeSource: corev1.VolumeSource{
